...
parser.add_argument("--material", help="material name to plot, e.g. 'A'.")
parser.add_argument("--seed", help="seed number to plot")
parser.add_argument("--fold", help="fold number to plot")
parser.add_argument("--title", nargs='?', const=1, type=int, help="False for no title")

args, optionals = parser.parse_known_args()
...
if args.seed is not None and args.fold is not None and args.material is not None:
    material = args.material
    seed = int(args.seed)
    fold = int(args.fold)
    print(f"User material {material}, seed {seed}, fold {fold}.")
    print(f"{args.title = }")

    materials_learning_curves = learning_curves.loc[
        (learning_curves["material"] == material) & (learning_curves["seed"] == seed)]

    number_epochs = len(materials_learning_curves.index)
    count_epochs = range(0, number_epochs)
    experiment_tag = best_seed_per_material_df.loc[best_seed_per_material_df["material"] == material]["tag"].to_string(
        index=False)
    ...
    plt.semilogy(count_epochs, materials_learning_curves[f"loss_trends_train_h_fold_{fold}"], label=f"train_h")
    plt.semilogy(count_epochs, materials_learning_curves[f"loss_trends_train_p_fold_{fold}"], label=f"train_p")
    plt.semilogy(count_epochs, materials_learning_curves[f"loss_trends_val_h_fold_{fold}"], label=f"val_h",
                marker="o", markersize=1)
    plt.semilogy(count_epochs, materials_learning_curves[f"loss_trends_val_p_fold_{fold}"], label=f"val_p",
                marker="o", markersize=1)
    plt.xlabel("Epochs")
    plt.ylabel("Training or validation loss")
    ...
    plt.tight_layout()
    # plt.show()
    path = PRED_SINK / f"LearnCurve_Exp_single_{args.experiment_id}_Material_{material}_Seed_{seed}_Fold_{fold}.pdf"
