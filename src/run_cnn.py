...
from utils.topology import TemporalAcausalConvNet
...
N_EPOCHS = 60 if DEBUG else 400  # how often should the full data set be iterated over
half_lr_at = [160, 300]  # halve learning rate after these many epochs
...
BATCH_SIZE = 4096  # how many periods/profiles/measurements should be averaged across for a weight update
...
def main():
    ...
    with Parallel(n_jobs=N_JOBS) as prll:
        for m_i, (material_lbl, mat_df) in enumerate(ds.groupby("material")):
            ...
            def run_dyn_training(rep=0):
                ...
                for kfold_lbl, test_fold_df in mat_df_proc.groupby("kfold"):
                    ...
                    if rep == 0 and kfold_lbl == 0 and m_i == 0:  # print only once
                        mdl_info = ti_summary(mdl, input_size=(1, len(x_cols)+1, train_tensor.shape[0]),
                    ...
                    for i_epoch in pbar:
                        ...
                        for i_batch in range(int(np.ceil(n_profiles / BATCH_SIZE))):
                            ...
                            opt.step()
                        with torch.no_grad():
                            logs["loss_trends_train"][kfold_lbl].append(train_loss.cpu().item())
                        ...
                        mdl.eval()
                        with torch.no_grad():
                            val_pred = mdl(val_tensor[:, :, :-1].permute(1, 2, 0)).permute(2, 0, 1)
                        ...
                        with torch.inference_mode():
                            ...
                            results_df.loc[
                                ...
                                freq=np.exp(val_tensor_numpy[0, :, 0].reshape(-1, 1))*150_000,
                                b=val_tensor_numpy[:, :, -2].T * b_limit,
                                h=val_pred.squeeze().cpu().numpy().T * h_limit,
                            )
            ...
            logs_d[material_lbl] = pd.DataFrame.from_dict([m['performance'] for m in mat_log])

    print("Overall Score")
    disp_d = pd.DataFrame({m: l.loc[:, 'avg-abs-rel-err'].to_numpy() for m, l in logs_d.items()})
    print(disp_d)
