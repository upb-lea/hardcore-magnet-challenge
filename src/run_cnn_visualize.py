    ...
    for m_i, (material_lbl, mat_df) in enumerate(ds.groupby("material")):
        ...
        def run_dyn_training(rep=0):
            ...
            for kfold_lbl, test_fold_df in mat_df_proc.groupby("kfold"):
                ...
                df_loss_per_model = pd.DataFrame(columns=['epoch', 'validation loss', 'training loss'])

                # Training loop
                for i_epoch in pbar:
                    # init model to detect training loss and validation loss curves over epoches
                    ...
                    for i_batch in range(int(np.ceil(n_profiles / BATCH_SIZE))):
                        ...
                        #print(f"{g_truth.shape = }")
                        X_tensor_ts = train_tensor_ts_shuffled_n_batched[:, :, :-1]
                        #print(f"{X_tensor_ts.shape = }")
                        X_tensor_scalar = train_tensor_scalar_shuffled_n_batched
                        #print(f"{X_tensor_scalar.shape = }")
                        output = mdl(
                            X_tensor_ts.permute(1, 2, 0), X_tensor_scalar
                        ).permute(2, 0, 1)
                        #print(f"{output.shape = }")

                        # change suggested in meeting on 2023-10-11: Till, Nikolas
                        #delta_p_loss = g_truth_ploss - get_bh_integral_from_two_mats(g_truth_b g_truth_h)
                        #train_loss = loss(output, delta_p_loss)

                        # original cost-function suggested by wilhelm
                    ...
                    df_epoch_to_append = pd.DataFrame(data={'epoch': i_epoch, 'validation loss': val_loss, 'training loss': train_loss.cpu().item()}, index=[0])
